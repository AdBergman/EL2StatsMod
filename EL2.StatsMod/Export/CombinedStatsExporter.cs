...
namespace EL2.StatsMod.Export
{
    internal static class CombinedStatsExporter
    {
        ...
        ///
        /// This DOES NOT serialize to JSON and does not write files.
        /// EndGameInfoExporter is responsible for serialization policy (camelCase).
        /// </summary>
        internal static AllStatsRoot Export(EmpireStatistics[] allEmpiresStatistics)
        {
            try
            {
                ...
                // Root "version" is set by EndGameInfoExporter on the combined file.
                return root;
            }
            catch (Exception ex)
            {
                // Don't crash the game if something goes wrong; just log and return null.
                StatsLoggerPlugin.Log?.LogError("[CombinedStatsExporter] Failed to build stats DTO: " + ex);
                return null;
            }
        }
    }
